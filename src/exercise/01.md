...
Example usage of `React.useReducer`

```js
const [state, dispatch] = React.useReducer(reducer, initialState)
```

The `reducer` function is called with two arguments

1. The current state
2. Whatever was passed to `dispatch` when called

The [Redux](https://redux.js.org/) pattern is to pass an object to the
`dispatch` function with at least a `type` property, set to a uppercase string,
for example
...
Then the `reducer` function has a big `switch` statement that returns a new
state based on the passed `type` property and throws in case an unrecognized
`type` is passed.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=01%3A%20useReducer%3A%20simple%20Counter&em=epicreact%40martynus.net).
