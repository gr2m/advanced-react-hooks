...
- `React.useImperativeHandle()` makes it possible for a component to expose
  internal APIs to its parent. The parent creates a `ref` and passes it to the
  component. In order for the component to accept a the ref as 2nd argument, it
  needs to be wrapped in
  `const MyComponent = React.forwardRef(function MyComponent(props, ref) {...})`.
  Now the `ref.current` value can be set to anything we want, e.g.
  ...
  The parent component can now call `ref.current.scrollToTop()`.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=05%3A%20useImperativeHandle%3A%20scroll%20to%20top%2Fbottom&em=epicreact%40martynus.net).
