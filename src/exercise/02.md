...
- `React.useCallback` is a hook that returns a memoized function. It's useful
  for memoizing expensive functions, but also useful when a function is a
  dependency for things like `React.useEffect` or `React.userReducer`. Without
  using `React.useCallback`, a function defined within a component would cause a
  re-render loop.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=02%3A%20useCallback%3A%20custom%20hooks&em=epicreact%40martynus.net).
