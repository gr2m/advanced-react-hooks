...
- When using `const MyContext = React.createContext()`, it's useful to create a
  custom component that returns `<MyContext.Provider value={...} {...props} />`
  and a custom `useMyContext()` function which calls
  `React.useContext(MyContext)` for better error messages in case the
  `useMyContext()` is called in a component outside of
  `<MyContext>...</MyContext>`
- Using context `React.createContext()` is tempting to avoid state drilling,
  meaning passing through state from one component to its children until the
  state is used. It's most often better to use component composition instead,
  e.g.
  ...
  return (
    <MyRootComponent>
      <MyChildComponent>
        <MyGrandChildComponent myState={myState}/>
      </MyChildComponent>
    </MyRootComponet>
  )
  ```

  Instead of `MyRootComponent` being in control of rendering its children and
  passing on the `myState` to the children that need it`, it just passes them
  through. That way a deep component tree can be composed as shown above,
  allowing to pass the state exactly to the component that needs it.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=03%3A%20useContext%3A%20simple%20Counter&em=epicreact%40martynus.net).
